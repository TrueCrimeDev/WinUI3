...
class RtTypeInfo {
    ...
    HasIActivationFactory {
        get {
            aa := this.m.ActivatableAttr
            if aa is Array {
                for token in aa
                    if _rt_Enumerator(53, this.m, "uint", this.t, "uint", token)(&_)
                        return true
                return false
            }
            return _rt_Enumerator(53, this.m, "uint", this.t, "uint", aa)(&_)
        }
    }
...
_rt_EnumAttrWithTypeArg(mdi, t, attr) {
    ...
    ; attr may be an array of MemberRef tokens (multiple constructor overloads).
    if attr is Array {
        ; Chain enumerators for each token.
        idx := 1
        inner := _rt_Enumerator_f(attrToType, 53, mdi, "uint", t, "uint", attr[1])
        next(&item) {
            loop {
                if inner(&item)
                    return true
                if ++idx > attr.Length
                    return false
                inner := _rt_Enumerator_f(attrToType, 53, mdi, "uint", t, "uint", attr[idx])
            }
        }
        return next
    }
